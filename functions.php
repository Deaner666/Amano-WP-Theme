...
function avia_title($post = "", $_product = "", $title = "")
{
	...
	// if(!is_front_page()) echo avia_breadcrumbs(); <--Removed by Dave
	if($title != "Home") { echo '<h1 class="page-title">'.$title.'</h1>'; }
...
// ADDED BY DAVE
/* Dave's simple title with no breadcrumb function (We're not
using this any more since I've just overwritten the above function */
function avia_simple_title($post = "", $_product = "", $title = "")
{
	if(!$_product)
	{
		global $product;
